...
void serial_time(double a, double b, double eps, float step, int num_of_time_samples = 1) {
	...
	double J_2;
	ofstream fout;
	fout.open("short2.txt", ios_base::app);
	for (int t = 1; t < num_of_time_samples + 1; t++) {
		J_2 = 10000000;

		std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();

		for (int n = 1; n < 10000000000; n += step) {
			dd = (b - a) / n;
			J_1 = J_2;
			J_2 = 0;
			for (int i = 1; i < n-1; i++) {
			...
			J_2 += edges;
			J_2 = dd*J_2;
			fout << n << ","<< (J_1 - J_2) / J_2 << ","<< J_2 << endl;
			if (log10(abs((J_1 - J_2)/J_2)) <= eps) break;
		}
...
void parallel_time(double a, double b, double eps, float step, int num_of_threads, int num_of_time_samples = 1) {
	...
	double minim = time[0];
	for (int y = 201000; y < num_of_time_samples; y++) {
		if (time[y] < minim) minim = time[y];
	}
...
int main() {
	...
	double eps[num_of_elem] = { -9, -10, -11, -12, -11, -11, -11 };
	float initial_step = 10;
